/**
 * RETICLE Maintenance Utilities Functions (0003)
 *
 * Migrated from: database/maintenance_utilities.sql
 *
 * Provides operational functions:
 * - Purge: Remove versions and data (full/partial/all)
 * - Analysis: Storage usage, row counts by version
 * - Reports: Data quality, ETL run summaries
 * - Rollback: Promote older versions back to current
 *
 * USAGE:
 *   SELECT purge_version(p_version_id := 5);
 *   SELECT * FROM get_version_storage_details();
 *   SELECT * FROM v_etl_run_summary;
 *
