...
class MaintenanceManager:
    ...
    def estimate_purge(self, version_id: int) -> bool:
        ...
        try:
            with self.db.get_session() as session:
                result = session.execute(text("""
                    SELECT * FROM estimate_purge_space(:version_id)
    ...
    def purge_version(self, version_id: int, confirm: bool = True) -> bool:
        ...
        try:
            with self.db.get_session() as session:
                result = session.execute(text("""
                    SELECT * FROM purge_version(:version_id)
                """), {'version_id': version_id})
    ...
    def promote_version(self, version_id: int) -> bool:
        ...
        try:
            with self.db.get_session() as session:
                result = session.execute(text("""
                    SELECT * FROM promote_version_to_current(:version_id)
                """), {'version_id': version_id})
    ...
    def show_etl_history(self) -> bool:
        ...
        try:
            with self.db.get_session() as session:
                result = session.execute(text("""
                    SELECT * FROM v_etl_pipeline_history LIMIT 20
                """))
                ...
                table_data = [
                    [
                        ...
                        row[1],  # version_id
                        row[2],  # organism
                        str(row[4])[:19],  # run_date
                        row[5],  # status
                        "✓" if row[6] else "",  # is_current
                        f"{row[7]:.1f}s" if row[7] else "N/A",  # duration
                        row[8][:40] if row[8] else ""  # error
                    ]
                    for row in rows
                ]
