...
# ─── CI role: SSO-only read access for injecting REACT_APP_SSO_* at build ──────
# Least privilege: this role reads ONLY the public SSO identifiers (client/tenant
# id), never the database or Secure-AI secrets, so a compromised web-build
# pipeline can't leak them. Created only when ci_principal_arns is non-empty.
