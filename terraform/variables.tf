...
variable "name_prefix" {
  description = "Name prefix grouping all secrets (e.g. RETICLE/<name>). Also used as the KMS alias/role-name stem and the Project/Category tag."
  type        = string
  default     = "RETICLE"
}
...
variable "role_name" {
  ...
  type        = string
  default     = "RETICLE-secrets-reader"
}
...
variable "ci_principal_arns" {
  description = "IAM principal ARNs (e.g. the GitHub Actions CI user) allowed to assume the SSO-only CI reader role. Empty disables the role."
  type        = list(string)
  default     = []
}

variable "ci_role_name" {
  description = "Name of the SSO-scoped CI reader role used to inject REACT_APP_SSO_* into the web build."
  type        = string
  default     = "RETICLE-sso-ci-reader"
}
