...
function run_gene(root, rxBody, APIBASE, locationSearch) {
  const exposed = [];
...
/* ---------- lookup ---------- */
/* How long to wait before calling it: the wiki answers in well under a second when the database
   is healthy, and a degraded RDS has been measured at 47 s. Without a deadline fetch() has none,
   so the page just kept saying "Assembling" until the user gave up. */
const LOOKUP_TIMEOUT_MS=20000;

/* One failure renderer for every way the lookup can fail, so the message always says which
   layer broke. The old code showed "Could not reach the server." for a server that had in fact
   answered — it read the body with r.json() BEFORE checking r.ok, and uvicorn returns its 500 as
   plain text, so the parse threw and every server-side failure fell into the network branch. */
function lookupFailed(g,msg,retry){
  const st=$('#status');
  st.style.display='block'; st.className='status err';
  st.innerHTML=esc(msg)+(retry?' <button class="retry-btn" id="lk-retry">Try again</button>':'');
  if(retry){ const b=$('#lk-retry'); if(b) b.onclick=()=>lookup(g); }
}
...
async function lookup(g){
  ...
  const timer=setTimeout(()=>{ if(ac) ac.abort(); }, LOOKUP_TIMEOUT_MS);
  let r;
  try{
    r=await fetch(`${APIBASE}/api/gene_wiki?gene=${encodeURIComponent(g)}&taxid=${taxid}`, ac?{signal:ac.signal}:undefined);
  }catch(e){
    clearTimeout(timer);
    /* Only a genuinely unreachable server lands here now. An abort is our own deadline firing,
       which means the server is up but the database behind it is not keeping up. */
    return lookupFailed(g, (e&&e.name==='AbortError')
      ? 'RETICLE is taking too long to answer — the gene database is likely under load. Nothing is wrong with '+g+'.'
      : 'Could not reach RETICLE. Check your connection.', true);
  }
  clearTimeout(timer);
  /* Read the body ONCE, as text, then try to parse. A 500 from uvicorn is the plain string
     "Internal Server Error"; parsing it first is what used to hide the real status code. */
  let body='', d=null;
  try{ body=await r.text(); d=JSON.parse(body); }catch(e){ d=null; }
  if(!r.ok){
    if(r.status===404) return lookupFailed(g, (d&&(d.detail||d.error))||('No gene called '+g+' in the knowledge base.'), false);
    if(r.status===422) return lookupFailed(g, (d&&(d.detail||d.error))||('"'+g+'" is not a valid gene symbol.'), false);
    /* 5xx: the server answered, so this is not a connectivity problem. Say so, and offer the
       retry rather than making the user retype the gene. */
    return lookupFailed(g, 'The gene database is not responding right now. This is not a problem with '+g+' — try again in a moment.', true);
  }
  if(!d||!d.identity) return lookupFailed(g, 'RETICLE returned a response it could not read for '+g+'.', true);
 ...
 *  and reuse short names ($, esc, fmt, COL, …) that would collide in a shared scope. */
function run_screen(root, rxBody, APIBASE, locationSearch) {
  const exposed = [];
  // See SCOPE_STUBS in the generator for why these shadow the real globals.
  const history = { replaceState() {}, pushState() {}, back() {}, forward() {}, go() {} };
  const location = { search: locationSearch || '', pathname: '/', hash: '', href: '', origin: window.location.origin };
 ...
 *  and reuse short names ($, esc, fmt, COL, …) that would collide in a shared scope. */
function run_network(root, rxBody, APIBASE, locationSearch) {
  const exposed = [];
  // See SCOPE_STUBS in the generator for why these shadow the real globals.
  const history = { replaceState() {}, pushState() {}, back() {}, forward() {}, go() {} };
  const location = { search: locationSearch || '', pathname: '/', hash: '', href: '', origin: window.location.origin };
...
async function loadContexts(){
  /* Unguarded this rejected on an unreachable API, and since nothing awaits loadContexts for its
     value that surfaced as an unhandled rejection with no context pills and no explanation. An
     empty list is the honest fallback: the default context still loads. */
  let j=null;
  try{ const r=await fetch(APIBASE+'/api/net_contexts?organism='+ORG); if(r.ok) j=JSON.parse(await r.text()); }
  catch(e){}
  CONTEXTS=(j&&j.contexts)||[];
...
async function load(){
  ...
  /* Body once as text, parsed after the status check — the same shape predictFn uses, and for the
     same reason. fetch does not reject on 4xx, and the API reports a refusal as `detail`, not
     `error`, so checking only j.error let a 404 fall through to j.nodes.map below and throw
     "Cannot read properties of undefined". The server's own sentence -- e.g. "'PEN' has no
     co-essentiality edges in this context" -- is the useful thing to show, and it was being
     replaced by a crash. detail is a string for our own aborts but an array for FastAPI's
     validation errors, so only a string is ever rendered. */
  let r,body;
  try{ r=await fetch(u); body=await r.text(); }catch(e){ status('request failed'); return; }
  let j=null; try{ j=JSON.parse(body); }catch(_){}
  const bad = !r.ok || !j || j.error || !Array.isArray(j.nodes) || !Array.isArray(j.edges);
  if(bad){
    const m = j && (j.error || j.detail);
    status(typeof m==='string' ? m
      : r.status>=500 ? 'RETICLE is not responding right now — try again in a moment.'
      : 'No network could be drawn for this gene.');
    if(CY)CY.elements().remove();renderPanel(null);return;
  }
...
function renderPanel(j){
  ...
  const wb=root.getElementById('widerbtn');
  if(wb) wb.onclick=()=>{ const cb=root.getElementById('recip'); cb.checked=false;
                          RECIP=false; load(); };
}
